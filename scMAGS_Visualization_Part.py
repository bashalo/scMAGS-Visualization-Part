# This code file is part of scmags package and created by Yusuf Baran
import os 
import scipy
import numpy as np
import pandas as pd
...
#-----------------------------------------------------------------------------#
    ...
    def __init__(
            self,
            data: Union[np.ndarray,scipy.sparse.csr_matrix, scipy.sparse.csc_matrix],
            labels: Union[np.ndarray],
            selected_markers: Union[dict],
            gene_ann: Union[np.ndarray] = None,
        ...
        # For provide old variables from other classes
        self._mark_keys = np.array(list(selected_markers.keys()))
        self._tot_iter = len(self._mark_keys)
        self._sel_list_markers = list(selected_markers.values())
        
        iters = [len(self._sel_list_markers[ky]) for ky in range(len(self._sel_list_markers))]
        max_mark = max(iters)
        
        marker_df = pd.DataFrame(self._sel_list_markers)
        marker_df.index = ["C_" + str(i) for i in self._mark_keys]  
        marker_df.columns = [
            "Marker_" + str(i) for i in range(1, max_mark + 1)
        ]    
        
        self._markers = marker_df
        
        # Visualization Class Defaults
